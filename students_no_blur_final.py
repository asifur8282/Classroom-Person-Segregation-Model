...
def process_students_with_yolo(image_data: np.ndarray, output_dir: str, start_index: int = 0):
    ...
    and saves square crops of each detected student without blurring.
    ...
    results = model(image_data,iou=0.4, conf=0.4,verbose=False)
    #results = model(image_data, iou= 0.01, conf= 0.4)
    ...
    for r in results:
        ...
        for i, box in enumerate(boxes):
            if int(box.cls[0]) == person_class_id:
                ...
                new_x1 = max(0, x1 - 10)
                new_y1 = max(0, y1)
                new_x2 = min(image_data.shape[1], new_x1 + square_side)
                new_y2 = min(image_data.shape[0], new_y1 + square_side)
                ...
                cropped_student = image_data[new_y1:new_y2, new_x1:new_x2]
